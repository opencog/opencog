 ...
 *
 * Similar to do_eval(), several important differences:
 * 1) The arugment must be SCM
...
SCM SchemeEval::do_scm_eval(SCM sexpr)
{
	...
	if (logger().isDebugEnabled())
	{
		...
		if (str && *str)
		{
			logger().debug("do_scm_eval(): Output: %s\n"
...
#ifdef LATER
/**
 * Evaluate the expression, returning a Handle. If an error occurs,
 * or if the result of the evaluation is not a handle, then 
 * Handle::UNDEFINED is returned.
 */
