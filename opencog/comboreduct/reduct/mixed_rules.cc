...
namespace reduct {
  ...
  void reduce_gt_zero_pair_power::operator()(combo_tree& tr,combo_tree::iterator it) const {
    ...
    typedef std::map<pre_it, bool,
      LADSUtil::lexicographic_subtree_order<vertex> > subtree_parity;
  ...
  void reduce_gt_zero_const_div::operator()(combo_tree& tr,combo_tree::iterator it) const {
    if(*it==id::greater_than_zero) {
      ...
      pre_it it_child = it.begin();
      if(*it_child==id::div) {
	LADSUtil::cassert(TRACE_INFO, it_child.number_of_children() == 2,
  ...
  void reduce_gt_zero_log::operator()(combo_tree& tr,combo_tree::iterator it) const {
    if(*it==id::greater_than_zero) {
      ...
      pre_it it_child = it.begin();
      if(*it_child==id::log) {
	LADSUtil::cassert(TRACE_INFO, it_child.has_one_child(),
  ...
  void reduce_impulse_power::operator()(combo_tree& tr,combo_tree::iterator it) const {
    typedef std::set<pre_it, LADSUtil::lexicographic_subtree_order<vertex>
      > subtree_quotient;
    if(*it==id::times) {
      subtree_quotient sq;
  ...
  void reduce_contin_if_to_impulse::operator()(combo_tree& tr,combo_tree::iterator it) const {
    if(*it==id::contin_if) {
      LADSUtil::cassert(TRACE_INFO, tr.number_of_children(it)==3,
  ...
  void reduce_contin_if::operator()(combo_tree& tr,combo_tree::iterator it) const {
	...
	*it = *b2;
	...
	LADSUtil::cassert(TRACE_INFO, tr.number_of_children(b1)==3, 
	...
	LADSUtil::cassert(TRACE_INFO, tr.number_of_children(b2)==3,
  ...
  void reduce_op_contin_if::operator()(combo_tree& tr,combo_tree::iterator it) const {
    ...
    if(*it==id::div) {
      LADSUtil::cassert(TRACE_INFO, tr.number_of_children(it)==2, 
	...
	LADSUtil::cassert(TRACE_INFO, tr.number_of_children(num)==3,
            "combo_tree node should have exactly two children (reduce_op_contin_if - num).");
	LADSUtil::cassert(TRACE_INFO, tr.number_of_children(denum)==3,
	...
	if(tr.equal_subtree(num_cond, denum_cond)) {
	  ...
	  *denum=id::div;
      ...
      for(sib_it sib = it.begin(); sib != it.end(); ) {
	if(*sib==id::contin_if) {
	  LADSUtil::cassert(TRACE_INFO, tr.number_of_children(sib)==3,
  ...
  void reduce_contin_if_inner_op::operator()(combo_tree& tr,combo_tree::iterator it) const {
	...
	if(b1_builtin==*b2) {
	  if(b1_builtin==id::div) {
	    LADSUtil::cassert(TRACE_INFO, tr.number_of_children(b1)==2,
                "combo_tree node should have exactly two children (reduce_contin_if_inner_op - b1).");
	    LADSUtil::cassert(TRACE_INFO, tr.number_of_children(b2)==2,
  ...
  bool reduce_from_assumptions::implies(const combo_tree& tr,
	...
	LADSUtil::cassert(TRACE_INFO, it1.has_one_child(),
	...
	if(*it1_child==id::greater_than_zero) {
	  LADSUtil::cassert(TRACE_INFO, it1_child.has_one_child(),
	...
	LADSUtil::cassert(TRACE_INFO, it2.has_one_child(),
	...
	if(*it2_child==id::greater_than_zero) {
	  LADSUtil::cassert(TRACE_INFO, it2_child.has_one_child(),
	...
	for(; assum != tr.end(); ++assum) {
	  if(implies(tr, pre_it(assum), it)) {
	    *it = id::logical_true;
	    ...
	    LADSUtil::cassert(TRACE_INFO, assum.has_one_child(),
  ...
  bool reduce_inequality_from_assumptions::gaussJordanElimination(double_matrix& dm, double eps) const {
    ...
    LADSUtil::cassert(TRACE_INFO, h>0, "Number of rows should be greater than 0 (gaussJordanElimitation)");
  ...
  void reduce_inequality_from_assumptions::gaussianElimination(double_matrix& dm) const {
    ...
    LADSUtil::cassert(TRACE_INFO, m>0, "Number of rows should be greater than 0 (gaussianElimitation)");
	...
	if(dm[maxi][j] != 0.0) {
	  ...
	  dm[maxi] = tmp_row;
	  double c = dm[i][j];
	  LADSUtil::cassert(TRACE_INFO, c != 0.0, "dm[%d][%d] == 0.0. Should be diferent", i, j);
  ...
  bool reduce_inequality_from_assumptions::gaussianElimination2(double_matrix& dm, double eps) const {
    ...
    LADSUtil::cassert(TRACE_INFO, m>0, "Number of rows should be greater than 0 (gaussianElimitation2)");
    int n = dm[m-1].size(); //number of columns
    LADSUtil::cassert(TRACE_INFO, n>0, "Number of columns should be greater than 0 (gaussianElimitation2)");
  ...
  bool reduce_inequality_from_assumptions::backSubstitution(double_matrix& dm, std::vector<double>& solution, double eps) const {
    LADSUtil::cassert(TRACE_INFO, solution.empty(), "There should not be any previous solution (backSubstitution)");
    int m = dm.size(); //number of rows
    LADSUtil::cassert(TRACE_INFO, m>0, "Number of rows should be greater than 0 (backSubstitution)");
    int n = dm[m-1].size(); //number of columns
    LADSUtil::cassert(TRACE_INFO, n>0, "Number of rows should be greater than 0 (backSubstitution)");
  ...
  void reduce_inequality_from_assumptions::operator()
    ...
    typedef std::map<pre_it, std::vector<contin_t>,
      LADSUtil::lexicographic_subtree_order<vertex> > subtree_row;
	...
	if(!assumptions.empty()) {
	  ...
	  for(unsigned int i = 0; i < assumptions.size(); i++) {
	    ...
	    LADSUtil::cassert(TRACE_INFO, *assum==id::greater_than_zero && assum.has_one_child(),
