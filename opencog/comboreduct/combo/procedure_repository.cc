...
namespace combo {
  ...
  void procedure_repository::add(procedure_call_base* pc) {
    ...
    if(spmi==_repo.end())
      ...
      LADSUtil::cassert(TRACE_INFO, *(spmi->second)==*pc, "A procedure named %s with a different body function has already been declared, you cannot overwrite it you must first explicitely remove it using the remove method", name.c_str());
    }
  }
  ...
  std::set<const procedure_call_base*> procedure_repository::procedure_call_dependencies(const procedure_call_base* pc) const {
    ...
    LADSUtil::cassert(TRACE_INFO, pc, "Not that must be an assumption, could return empty set as well");
  ...
  bool procedure_repository::infer_types_repo() {
	...
	for(procedure_call_set_it pci = sci->begin();
	  ...
	  for(pre_it it = body.begin(); it != body.end(); ++it) {
		...
		LADSUtil::cassert(TRACE_INFO, false, error_message.c_str());
	      }
		...
		if(pt_it==pt.end()) {
		  ...
		  LADSUtil::cassert(TRACE_INFO, pa==ia, "The arity of %s has been contextually infered at %d which is different from the previously given or infered arity %d", pc->get_name().c_str(), ia, pa);
		  ...
		  if(pa!=ia)
		    printf("The arity of %s has been contextually infered at %d which is different from the previously given or infered arity %d\n", pc->get_name().c_str(), ia, pa);
		  LADSUtil::cassert(TRACE_INFO, ia==pa, "The arity of %s has been contextually infered at %d which is different from the previously given or infered arity %d", pc->get_name().c_str(), ia, pa);
		  pc->set_type_tree(tt);
	...
	if(inherit_type_tree(tt, pt[pc])) {
	  ...
	  arity_t ia = type_tree_arity(tt);
	  pc->set_type_tree(tt);
	  LADSUtil::cassert(TRACE_INFO, ia==pa,
	  ...
	  LADSUtil::cassert(TRACE_INFO, false, message.str().c_str());
	}
      }
    }
