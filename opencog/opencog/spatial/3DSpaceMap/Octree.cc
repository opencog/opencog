...
void Octree::addSolidBlock(Block3D * _block, bool byKnownIndexes, int _x, int _y, int _z)
{
    ...
    // until here, we've got the parent tree of this block and its child indexesrem of this block.
...
Handle Octree::removeAnUnitSolidBlock(BlockVector& _pos)
{
    ...
    int x,y,z;
	//Note: for now there's not any case which unit block inside a big block
    ...
    while(true)
    {
        ...
        else
        {
            ...
            else if (tree->mAllMyBlocks[x][y][z] != 0)
            {
				...
				logger().error("in getparenttree, isInsideAbigblock is True, and the big block address is %p,%l",(void*)bigBlock,bigBlock);
                return tree;
            }
            else
            {
