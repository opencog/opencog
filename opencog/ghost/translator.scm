...
; ----------
(define (process-action ACTION RULENAME IS-REJOINDER?)
"
                     ...
                     (if (null? reused-rule-from-alist)
                       ...
                       (begin
                         ...
                         (process-action
                           ...
                           (list-ref reused-rule-from-alist 1) label IS-REJOINDER?))))
                   (begin
                     (set! reuse #t)
                     (set! reused-rule-label label)
...
"
  ...
  (let* ((proc-type (process-type TYPE))
         ...
         (action (process-action ACTION NAME is-rejoinder?))
