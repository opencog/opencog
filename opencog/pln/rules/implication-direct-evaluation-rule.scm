...
(define (implication-direct-evaluation-formula P Q)
  (let* (
    ...
    ;; (cog-logger-info "[PLN-Induction] P = ~a" P)
    ;; (cog-logger-info "[PLN-Induction] Q = ~a" Q)
    ;; (cog-logger-info "[PLN-Induction] P-instances = ~a" P-instances)
    ;; (cog-logger-info "[PLN-Induction] Q-instances = ~a" Q-instances)
    ;; (cog-logger-info "[PLN-Induction] P-length = ~a" P-length)
    ;; (cog-logger-info "[PLN-Induction] P-inter-Q-length = ~a" P-inter-Q-length)
    ;; (cog-logger-info "[PLN-Induction] TV-strength = ~a" TV-strength)
    ;; (cog-logger-info "[PLN-Induction] TV-confidence = ~a" TV-confidence)
