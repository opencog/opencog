...
{
    // Retrieve the atoms in the AttentionalFocus, excluding this source atom
    ...
    // Get the neighboring atoms, where the connecting edge is an
    // AsymmetricHebbianLink in either direction
    ...
    // Get the set differences between the AttentionalFocus and the sets of
    // existing sources and targets
    ...
    // Resulting in the sets of nodes that require a new AsymmetricHebbianLink
    // in either direction
    foreach (Handle node, needToBeSource) {
        as->addLink(ASYMMETRIC_HEBBIAN_LINK, node, source, SimpleTruthValue::createTV(0, 1));
    }

    foreach (Handle node, needToBeTarget) {
        as->addLink(ASYMMETRIC_HEBBIAN_LINK, source, node, SimpleTruthValue::createTV(0, 1));
    }
}
