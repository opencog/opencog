...
```
   echo -e "(observe-text \"this is a another test\" \"lg\" 24)" |nc localhost 17005
   echo -e "(observe-text \"Bernstein () (1876\" \"lg\" 24)" |nc localhost 17005
   echo -e "(observe-text \"Lietuvos žydų kilmės žurnalistas\" \"lg\" 24)" |nc localhost 17005
```
...
7) Finally, there are some parameters you can optionally adjust before starting
   ...
   the scripts in the `run` and `scm` directories, in particular the use of the
   `params.txt` configuration file (more on it will come in the next sections).
...
4) If you are familiar with the counting and parsing methods used in the
   ...
   pipeline, open `configs/params.txt` and choose the counting mode you want to use.
   Otherwise, just leave the default values:

     - clique: adds one count to each word-pair within distance `cnt_reach` 
               of the current word
     - clique-dist: adds wighted counts to each word-pair within distance
                    `cnt_reach` of the current word. The weight is given by
                    floor(cnt_reach/separation), where separation is the 
                    distance between the words in the word-pair in the sentence
     - lg: adds one count to each word-pair linked in each of the cnt_reach
