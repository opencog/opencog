...
(define-method (get-lexical-node (ni <noun-item>))
	(define (determine-lexical)
		...
		(receive (supersets subsets)
			...
			(set! subsets (filter (lambda (l) (and (> (cog-tv-mean (cog-tv l)) 0.5) (> (cog-tv-conf (cog-tv l)) 0.5))) subsets))

			(let* ((weights
				; calculate a weight for each link
				(map (lambda (l) (length (cog-incoming-set (gar l))) * (cog-tv-mean (cog-tv l)) * (cog-tv-conf (cog-tv l))) subsets))
			       (sorted-zip
