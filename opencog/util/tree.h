...
class upwards_iterator : public iterator_base
    ...
    upwards_iterator begin_upwards(iter it) const {
        return upwards_iterator(it);
    }
    /// Return upwards iterator to above the root of the tree.
    upwards_iterator end_upwards() const {
        return upwards_iterator(NULL);
    }
...
void tree<T, tree_node_allocator>::validate(const iterator_base& it) const {
    ...
    if (n->parent) {
        if (!n->prev_sibling)
            assert(n->parent->first_child == n);
        if (!n->next_sibling)
            assert(n->parent->last_child == n);
    }
