...
class iObject
{
 ...
 public:
	iMovable(iMeshWrapper* mw) { mesh = mw; }
	iMovable(csVector3 _pos) : pos(_pos) {}
	csVector3 GetPosition() const { return pos; }
	csVector3 GetFullPosition() const { return pos; }
	void SetPosition(csVector3 _pos) { pos = _pos; }
	csVector3 GetRotation() const { return rot; }
	void SetRotation(csVector3 _rot) { rot = _rot; }
	void UpdateMove();
 ...
 public:
	 ...
	 iMeshWrapper() { movable = csRef<iMovable>(new iMovable(this)); obj = csRef<iObject>(new iObject); }
	 iMovable* GetMovable() const { return movable.get(); }
	iObject* QueryObject() const { return obj.get(); }
	iMeshWrapper* GetMeshObject() { return this; }
	iMeshWrapper* GetObjectModel() { return this; }
	void GetObjectBoundingBox(csBox3& box) const { box = csBox3(-dim.x/2,dim.x/2,-dim.y/2,dim.y/2, -dim.z/2, dim.z/2); }
 };
 ...
 public:
	 iMeshWrapper* FindByName(string name) { return STLhas(meshes, name) ? meshes[name].get() : NULL; }
	 int GetCount() const { return meshes.size(); }
...
//	std:: map<csVector3,csRef<iMeshWrapper> > pos2mesh;
 ...
 public:
	iEngine();
	iMeshList* GetMeshes () { return meshes.get(); }
	void UpdateMove(iMovable* m);
	iMeshWrapper* FindMeshObject(string name) { return meshes->FindByName(name); }
...
//		pos2mesh[pos] = mesh;
	...
	void SetAmbientLight(csVector3 _amb) { amb=_amb; }
	void Prepare() {}
	void RemoveObject(iMeshWrapper* m) { meshes->Remove(m); }
...
//	map<int, map<int, iMeshWrapper*> > VisibleObjects(csVector3 orig, float sphi);
 ...
 public:
	 iMeshWrapper* mesh;
	 csColliderWrapper(iMeshWrapper* _mesh) { mesh = _mesh; }
 };
...
public:
	...
	bool InitcsPlugins(	iGUIProvider* _GUIprovider ) { GUIprovider = _GUIprovider; return true; };

	/** Initialize CS after the plugin system is up. */
	CSstatus* OpenMainSystem() { return &status; }
