...
namespace MessagingSystem {
  ...
  ImitationLearningTask::ImitationLearningTask() : _lts(LTS_IDLE),
						   _entropyFilter(NULL),
    ...
    MAIN_LOGGER.log(opencog::Logger::INFO, "ImitationLearningTask - Created random number generator (%p) for ImitationLearningTask with seed %lu", _rng, rand_seed);
  ...
  //command methods
  bool ImitationLearningTask::initLearning(int nepc,
					   WorldProvider* wp,
    ...
    if(_BDCat.empty()) {
      MAIN_LOGGER.log(opencog::Logger::INFO, "ImitationLearningTask - All exemplars are empty, learning will not start.");
      ...
      //----------------------------------------
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - Getting definite objects from SpaceMap.");
	...
	MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - List of the definite objects used for learning");
	for(std::set<std::string>::iterator si = _definite_objects.begin();
	  ...
	  MAIN_LOGGER.log(opencog::Logger::DEBUG,
			  "ImitationLearningTask - %s", si->c_str());
	}
      }
      //~debug log
      ...
      //debug log
      if(opencog::Logger::DEBUG <= MAIN_LOGGER.getLevel()) {
	MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - List of the messages used for learning");
	for(message_set_const_it mi = _messages.begin();
	    mi != _messages.end(); ++mi) {
	  MAIN_LOGGER.log(opencog::Logger::DEBUG,
			  "ImitationLearningTask - %s",
			  mi->getContent().c_str());
	}
      }
      //~debug log      
      ...
      //debug log
      if(opencog::Logger::DEBUG <= MAIN_LOGGER.getLevel()) {
	MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - List of the agent and action definite objects used for learning");
	for(agent_to_actions_const_it atas_it = _atas.begin();
	    atas_it != _atas.end(); ++atas_it) {
	  MAIN_LOGGER.log(opencog::Logger::DEBUG,
			  "ImitationLearningTask - Agent : %s",
			  atas_it->first.c_str());
	  ...
	  for(definite_object_vec_set_const_it ados_it = ados.begin();
		...
		dci != ados_it->end(); ++dci)
	      s += *dci + std::string(" ");
	    MAIN_LOGGER.log(opencog::Logger::DEBUG,
			    "ImitationLearningTask - Actions and arguments : %s",
			    s.c_str());
	  }
	}
      }
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, start processing.");
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, stop processing.");

      //debug log
      if(opencog::Logger::DEBUG <= MAIN_LOGGER.getLevel()) {
	MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - List of perceptions filtered by EntropyFilter");
	for(combo_tree_ns_set_const_it pi = _atomic_perceptions.begin();
	    pi != _atomic_perceptions.end(); ++pi) {
	  stringstream ss;
	  ss << *pi;
	  MAIN_LOGGER.log(opencog::Logger::DEBUG,
			  "ImitationLearningTask - %s", ss.str().c_str());
	}
      }
      //~debug log

      //compute and filter actions
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - ActionFilter, start processing.");
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - ActionFilter, stop processing.");

      if(_atomic_actions.size() > 0) {
	
	//debug log
	if(opencog::Logger::DEBUG <= MAIN_LOGGER.getLevel()) {
	  MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - List of actions filtered by ActionFilter");
	...
	MAIN_LOGGER.log(opencog::Logger::DEBUG,
			"ImitationLearningTask - No action filtered by ActionFilter, learning will not start");
	return false;
      }
	...
	new FE(wp, _pet_id, _owner_id, _avatar_id, _trick_name,
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG,
		      "ImitationLearningTask - SPCTools - New estimator");
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - definite object count : %d", _definite_objects.size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - indefinite object count : %d", io.size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - elementary operator count : %d", eo.size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic perception count : %d", _atomic_perceptions.size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic action count : %d", _atomic_actions.size());
      //~debug log for SPCTools
  ...
  void ImitationLearningTask::addLearningExample(WorldProvider* wp,
						 const argument_list& al) {
    opencog::cassert(TRACE_INFO, wp, "The World Provider points to NULL");
    //add new exemplar
    ...
    if(cat_count!=_BDCat.getSize()) { //a new exemplar has been added
      ...
      //debug log
      if(opencog::Logger::DEBUG <= MAIN_LOGGER.getLevel()) {
	MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - List of definite objects used for learning");
	for(std::set<std::string>::const_iterator si =
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG,
		      "ImitationLearningTask - Add Learning Example.");
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, start update processing.");
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, stop update processing.");
	...
	for(combo_tree_ns_set_const_it ai = _atomic_actions.begin();
      ...
      MAIN_LOGGER.log(opencog::Logger::DEBUG,
		      "ImitationLearningTask - SPCTools - New estimator");
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - definite object count : %d", _definite_objects.size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - indefinite object count : %d", _PVoc->get_indefinite_objects().size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - elementary operator count : %d", _PVoc->get_elementary_operators().size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic perception count : %d", _atomic_perceptions.size());
      MAIN_LOGGER.log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic action count : %d", _atomic_actions.size());
      //~debug log for SPCTools
  ...
  void ImitationLearningTask::waitForReward() {
    _lts = LTS_IDLE;
  }

  void ImitationLearningTask::setFitness(fitness_t f) {
	  opencog::cassert(TRACE_INFO, _PIL!=NULL);
    _PIL->set_current_fitness(f);
    _lts = LTS_LEARN;
  }

  void ImitationLearningTask::stopLearning() {
	  opencog::cassert(TRACE_INFO, _PIL!=NULL);
  ...
  const combo_tree& ImitationLearningTask::getBestSchema() {
	  opencog::cassert(TRACE_INFO, _PIL!=NULL);
  ...
  const combo_tree& ImitationLearningTask::getBestSchemaEstimated() {
	  opencog::cassert(TRACE_INFO, _PIL!=NULL);
