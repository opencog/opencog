/*
 * opencog/embodiment/Learning/LearningServer/ImitationLearningTask.cc
 *
        ...
        _PIL(NULL), _PVoc(NULL)
{
    ...
    logger().log(opencog::Logger::INFO, "ImitationLearningTask - Created random number generator (%p) for ImitationLearningTask with seed %lu", _rng, rand_seed);
        ...
        const string& trick_name)
{
    ...
    if (_BDCat.empty()) {
        logger().log(opencog::Logger::INFO, "ImitationLearningTask - All exemplars are empty, learning will not start.");
        ...
        //----------------------------------------
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - Getting definite objects from SpaceMap.");
        ...
        if (opencog::Logger::DEBUG <= logger().getLevel()) {
            logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - List of the definite objects used for learning");
        ...
        //debug log
        if (opencog::Logger::DEBUG <= logger().getLevel()) {
            logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - List of the messages used for learning");
        ...
        //debug log
        if (opencog::Logger::DEBUG <= logger().getLevel()) {
            logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - List of the agent and action definite objects used for learning");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, start processing.");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, stop processing.");

        //debug log
        if (opencog::Logger::DEBUG <= logger().getLevel()) {
            logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - List of perceptions filtered by EntropyFilter");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - ActionFilter, start processing.");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - ActionFilter, stop processing.");

        if (_atomic_actions.size() > 0) {

            //debug log
            if (opencog::Logger::DEBUG <= logger().getLevel()) {
                logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - List of actions filtered by ActionFilter");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - definite object count : %d", _definite_objects.size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - indefinite object count : %d", io.size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - elementary operator count : %d", eo.size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic perception count : %d", _atomic_perceptions.size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic action count : %d", _atomic_actions.size());
        //~debug log for SPCTools
        ...
        const argument_list& al)
{
    ...
    if (cat_count != _BDCat.getSize()) { //a new exemplar has been added
        ...
        //debug log
        if (opencog::Logger::DEBUG <= logger().getLevel()) {
            logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - List of definite objects used for learning");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, start update processing.");
        ...
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - EntropyFilter, stop update processing.");
        ...
        logger().log(opencog::Logger::DEBUG,
                     "ImitationLearningTask - SPCTools - New estimator");
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - definite object count : %d", _definite_objects.size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - indefinite object count : %d", _PVoc->get_indefinite_objects().size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - elementary operator count : %d", _PVoc->get_elementary_operators().size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic perception count : %d", _atomic_perceptions.size());
        logger().log(opencog::Logger::DEBUG, "ImitationLearningTask - SPCTools - atomic action count : %d", _atomic_actions.size());
        //~debug log for SPCTools
