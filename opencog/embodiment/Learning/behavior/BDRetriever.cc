...
namespace behavior {
  ...
  void BDRetriever::addLastExemplar(BehaviorCategory& bc,
    ...
    if(bd.empty()) { //if the exemplar is empty it is simply ignored
      MAIN_LOGGER.log(opencog::Logger::INFO, "BDRetriever - The last exemplar is empty it will simply be ignored.");
    }
    else {
      bc.addCompositeBehaviorDescription(bd);
      est.push_back(et);
      MAIN_LOGGER.log(opencog::Logger::INFO, "BDRetriever - The last exemplar %s has been retrieved successfully.", bd.toString().c_str());
    }
  }
...
void BDRetriever::retrieveAllExemplars(BehaviorCategory& bc,
    ...
    if(h != Handle::UNDEFINED) {
        ...
        for(std::list<HandleTemporalPair>::iterator ip = retP.begin(); ip != retP.end(); ++ip) {
            ...
            retrieveExemplar(bd, wp, h, temp);
            if(bd.empty()) {
              MAIN_LOGGER.log(opencog::Logger::INFO, "BDRetriever - The exemplar of interval [%u, %u] is empty it will simply be ignored.", temp.getLowerBound(), temp.getUpperBound());
            }
            else {
              bc.addCompositeBehaviorDescription(bd);
	      est.push_back(temp);
              MAIN_LOGGER.log(opencog::Logger::INFO, "BDRetriever - The following exemplar %s has been retrieved successfully.", bd.toString().c_str());
            }
        }
    }
}
