...
float EvaluationLinkSimilarityEvaluator::similarity(AtomSpace& atomSpace, Handle evalLinkHandle1, Handle evalLinkHandle2)
{
    ...
    if ((atomSpace.get_arity(evalLinkHandle1) < 2) || (atomSpace.get_arity(evalLinkHandle2) < 2)) {
        return 0;
    }

    Handle listLink1 = atomSpace.get_outgoing(evalLinkHandle1, 1);
    Handle listLink2 = atomSpace.get_outgoing(evalLinkHandle2, 1);

    //printf("llarity1 = %d\n", listLink1->getArity());
    //printf("llarity2 = %d\n", listLink1->getArity());
    if ((atomSpace.get_arity(listLink1) != atomSpace.get_arity(listLink2)) || (atomSpace.get_arity(listLink1) < 2)) {
        return 0;
    }
    ...
    float answer = 1;
    for (int i = atomSpace.get_arity(listLink1) - 1; i > 0; i--) {
        Handle h1 = atomSpace.get_outgoing(listLink1, i);
        Handle h2 = atomSpace.get_outgoing(listLink2, i);
...
float EvaluationLinkSimilarityEvaluator::computeHandleSimilarity(AtomSpace& a, Handle h1, Handle h2)
{
    ...
    Handle link_handle;
    HandleSeq set = a.get_incoming(h1);
    ...
    for (si = set.begin(); (! found) && (si != set.end()); ++si) {
        ...
        Handle sh = *si;
        if (classserver().isA(a.get_type(sh), SIMILARITY_LINK)) {
            std::vector<Handle> outgoing = a.get_outgoing(sh);
