...
//    spaceMap.getMetaData( atomSpace.getName(agent) );
  ...
  if ( !limitLine.intersects( leftLine, &intersectionPoint ) ) {
    MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PetPsychePredicatesUpdater - Lines should intersect limitLine(%s) leftLine(%s)", limitLine.toString( ).c_str( ), leftLine.toString( ).c_str( ) );
  } // if
  ...
  if ( !limitLine.intersects( rightLine, &intersectionPoint ) ) {
    MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PetPsychePredicatesUpdater - Lines should intersect limitLine(%s) rightLine(%s) fovDirection(%s) reach(%f) maxPoint(%s)", limitLine.toString( ).c_str( ), rightLine.toString( ).c_str( ), agentFovDirection.toString( ).c_str( ), agentFovReach, maximumPoint.toString( ).c_str( ) );
  } // if
  ...
  if ( !Spatial::Math::lineIntersection( limitLine, Spatial::Math::LineSegment( Spatial::Math::Vector3( 0,0 ), leftPoint ), intersectionPoint ) ) {
    MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PetPsychePredicatesUpdater - Line intersection cannot be null lineA(%f,%f - %f, %f) lineB(%f,%f - %f,%f).", limitLine.pointA.x, limitLine.pointA.y, limitLine.pointB.x, limitLine.pointB.y, 0, 0, leftPoint.x, leftPoint.y );
  } // if
  

  leftPoint = intersectionPoint;
  if ( !lineIntersection( limitLine, Spatial::Math::LineSegment( Spatial::Math::Vector3( 0,0 ), rightPoint ), intersectionPoint ) ) {
    MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PetPsychePredicatesUpdater - Line intersection cannot be null lineA(%f,%f - %f, %f) lineB(%f,%f - %f,%f).", limitLine.pointA.x, limitLine.pointA.y, limitLine.pointB.x, limitLine.pointB.y, 0, 0, rightPoint.x, rightPoint.y );
  } // if
  rightPoint = intersectionPoint;
  */
...
void PetPsychePredicatesUpdater::update(Handle object, Handle pet, unsigned long timestamp ) {
  MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PetPsychePredicatesUpdater - updating context predicates..." );
  
  if ( pet == Handle::UNDEFINED ) {
    MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PetPsychePredicatesUpdater - Got an undefined handle for the pet." );    
    return;
  } // if
  ...
  if (spaceMapHandle == Handle::UNDEFINED) {
    MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PetPsychePredicatesUpdater - there is no space map defined at this moment..." );
    return;
  } // if
  ...
  if ( !spaceMap.containsObject(petName) ) {
    MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PetPsychePredicatesUpdater - Pet was not inserted in the map yet." );    
    return;
  }
  ...
  foreach( std::string entity, entities ) {
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - inspecting entity %s", entity.c_str( ) );
    Handle entityHandle = getHandle(entity);
    if( entityHandle == Handle::UNDEFINED ) {
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - there is no entity %s defined at atomspace", entity.c_str( ) );
      continue;
    } // if
    ...
    { // log pet and element positions
      ...
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, message.str( ).c_str( ) );
    } // end block
	...
	if ( petFieldOfView.isInside( bottomSegments[i].pointA ) ){ // pointB is the pointA of the next segment
      ...
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - %s is inside pet fov? %s", entity.c_str( ), ( meanValue ? "y" : "n" ) );
    } // if
    ...
    if ( atomSpace.getType(entityHandle) == SL_AVATAR_NODE ) {      
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - entity %s is an avatar", entity.c_str( ) );
	...
	nextOwner |= isOwner;
      ...
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - Entity %s properties: owner[%s] friend[%s] enemy[%s]", entity.c_str( ), ( isOwner ? "t" : "f" ), ( isFriend ? "t" : "f" ), ( isEnemy ? "t" : "f" ) );

    } else if ( atomSpace.getType(entityHandle) == SL_OBJECT_NODE ) {

      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - entity %s is an object", entity.c_str( ) );
	...
	nearPeePlace |= isPeePlace;
      ...
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - Entity %s properties: edible[%s] drinkable[%s] pooPlace[%s] peePlace[%s]", entity.c_str( ), ( isEdible ? "t" : "f" ), ( isDrinkable ? "t" : "f" ), ( isPooPlace ? "t" : "f" ), ( isPeePlace ? "t" : "f" ) );
  ...
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - current time: day[%d] month[%d] year[%d] hour[%d] min[%d] sec[%d]", timeInfo.tm_mday, timeInfo.tm_mon+1, timeInfo.tm_year+1900, timeInfo.tm_hour, timeInfo.tm_min, timeInfo.tm_sec );  
  ...
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is at home: %s", (atHome ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is outside home: %s", (!atHome ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near owner: %s", (nearOwner ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near friend: %s", (nearFriend ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near enemy: %s", (nearEnemy ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near food: %s", (nearFood ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near water: %s", (nearWater ? "t" : "f" ) );

  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near poo place: %s", (nearPooPlace ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is near pee place: %s", (nearPeePlace ? "t" : "f" ) );
  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - pet is next to owner (Used on learning mode): %s", (nextOwner ? "t" : "f" ) );

  MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PetPsychePredicatesUpdater - is at night: %s", (atNight ? "t" : "f" ) );

  MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PetPsychePredicatesUpdater - context predicates updated." );
