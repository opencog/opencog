...
#define XSD_FILE_NAME "BrainProxyAxon.xsd" 
    ...
    if (schemaLocation.size() > 0) {
        MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PAI - Setting the schemaLocation to: %s\n", schemaLocation.c_str());
...
ActionPlanID PAI::createActionPlan() {
    ...
    planId = LADSUtil::toString(nextActionPlanId);
    nextActionPlanId++;
#ifdef HAVE_LIBPTHREAD
...
void PAI::sendActionPlan(ActionPlanID planId) throw (LADSUtil::RuntimeException, std::bad_exception) {
...
void PAI::sendEmotionalFeelings(const std::string& petId, const std::map<std::string, float>& feelingsValueMap){
    ...
    for(it = feelingsValueMap.begin(); it != feelingsValueMap.end(); it++){
        ...
        XMLCh* valueStr = XERCES_CPP_NAMESPACE::XMLString::transcode(LADSUtil::toString(it->second).c_str());
...
ActionID PAI::addAction(ActionPlanID planId, const PetAction& action) throw (LADSUtil::RuntimeException, LADSUtil::InvalidParamException, std::bad_exception) {
    ...
    if (it != inProgressActionPlans.end()) {
        ...
        MAIN_LOGGER.log(LADSUtil::Logger::WARNING, "PAI - No action plan with id = %s in progress\n", planId.c_str());
...
bool PAI::processPVPMessage(const string& pvpMsg, HandleSeq &toUpdateHandles) {
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPVPMessage.");

    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPVPMessage atomTable.size=%d", atomSpace.getAtomTable().getSize() );

    if (logPVPMessage) {
        MAIN_LOGGER.log(LADSUtil::Logger::INFO, " PAI - Processing PVP message:\n%s\n", pvpMsg.c_str());
    }
    ...
    try {
        ...
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - XML Exception: %s\n", message);
...
//        XERCES_CPP_NAMESPACE::XMLString::release(&heightStr);
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - DOM Exception: %s\n", message);
        XERCES_CPP_NAMESPACE::XMLString::release(&message);
        delete memBufIS;
        //delete parser;
        return false;
        ...
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Unexpected XML Parse Exception\n");
        delete memBufIS;
        //delete parser;
        return false;
    }   
    ...
    if(parser->getErrorCount() == 0)
    {
        ...
        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - DOMDocument retrieved");
        
        try {
            processPVPDocument(document, toUpdateHandles);
            MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPVPDocument done");
...
void PAI::processPVPDocument(XERCES_CPP_NAMESPACE::DOMDocument * doc, HandleSeq &toUpdateHandles){
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPVPDocument");
    XMLCh tag[PAIUtils::MAX_TAG_LENGTH+1];
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing map-info done");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing pet-signal done");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing avatar-signal done");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing agent-signal done");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing object-signal done");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing instructions done");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Processing agent-sensor-info done");



}

void PAI::processAgentSignal(XERCES_CPP_NAMESPACE::DOMElement * element) throw (LADSUtil::RuntimeException, LADSUtil::InvalidParamException, std::bad_exception) {
    XMLCh tag[PAIUtils::MAX_TAG_LENGTH+1];
    ...
    if (!setLatestSimWorldTimestamp(tsValue)) {
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Received old timestamp in agent-signal => Message discarded!");
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Got agent-signal: agentId = %s (%s), name = %s, timestamp = %s\n", agentID, internalAgentId.c_str(), name, timestamp);
    ...
    if ( agentTypeStr == AVATAR_OBJECT_TYPE ) {
      ...
      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - agent is an humanoid: [%s] -> (%s)\n", agentID, internalAgentId.c_str() );
    } else {
    ...
    for(unsigned int i = 0; i < list->getLength(); i++){
        ...
        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Got param: name = %s, type = %s, value = %s", paramName, paramType, paramValue);
            ...
            case VECTOR_CODE:
                {
                    ...
                    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Got vector: x = %s, y = %s, z = %s\n", xStr, yStr, zStr);
            ...
            case ROTATION_CODE:
                {
                    ...
                    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Got rotaion: pitch = %s, roll = %s, yaw = %s\n", pitchStr, rollStr, yawStr);
...
void PAI::processPetSignal(XERCES_CPP_NAMESPACE::DOMElement * element){
    ...
    XERCES_CPP_NAMESPACE::XMLString::transcode(TIMESTAMP_ATTRIBUTE, tag, PAIUtils::MAX_TAG_LENGTH);
    char* timestamp = XERCES_CPP_NAMESPACE::XMLString::transcode(element->getAttribute(tag));
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI:processPetSignal - timestamp: %s", timestamp);
    unsigned long tsValue = getTimestampFromXsdDateTimeStr(timestamp);
    if (!setLatestSimWorldTimestamp(tsValue)) {
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Received old timestamp in pet-signal => Message discarded!");
        XERCES_CPP_NAMESPACE::XMLString::release(&timestamp);
	return;
    }
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI - pet-id %s (%s), name: %s, status: %s, statusCode: %d", petID, internalPetId.c_str(), name, status, statusCode);
...
//            MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPetSignal - before addPhysiologicalFeelingParam.");
            Handle paramListLink = addPhysiologicalFeelingParam(paramName, paramValue);
            feelingParams.push_back(paramListLink);

            XERCES_CPP_NAMESPACE::XMLString::release(&paramName);
            XERCES_CPP_NAMESPACE::XMLString::release(&paramValue);
//            MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPetSignal - after addPhysiologicalFeelingParam.");
        }

//        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPetSignal - before addPhysiologicalFeeling.");
        addPhysiologicalFeeling(internalPetId.c_str(), name, tsValue, feelingParams);
//        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPetSignal - after addPhysiologicalFeeling.");

    } else {
    	
//    	MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPetSignal - before send action plan.");
...
//      MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processPetSignal - after send action plan.");
    }
...
bool PAI::setLatestSimWorldTimestamp(unsigned long timestamp) {
    if (timestamp > latestSimWorldTimestamp) {
        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - setLatestSimWorldTimestamp(%lu).", timestamp, latestSimWorldTimestamp);
        latestSimWorldTimestamp = timestamp;
    } else if (timestamp < latestSimWorldTimestamp) {
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - setLatestSimWorldTimestamp(%lu): Got a timestamp smaller than the latest received timestamp (%lu)!", timestamp, latestSimWorldTimestamp);
	return false;
    }
    return true;
}

unsigned long PAI::getTimestampFromXsdDateTimeStr(const char* xsdDateTimeStr) throw (LADSUtil::RuntimeException, std::bad_exception) {
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI - getTimestampFromXsdDateTimeStr(%s)", 
                    xsdDateTimeStr);
    ...
    try {
        ...
        throw LADSUtil::RuntimeException(TRACE_INFO, "PAI - Invalid regular expression (%s).\n %s",
    ...
    //MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - ulong timestamp = %lu, converted from %s", result, xsdDateTimeStr);
    return result;
}
...
void PAI::processInstruction(XERCES_CPP_NAMESPACE::DOMElement * element){
    ...
    unsigned long tsValue = getTimestampFromXsdDateTimeStr(timestamp);
    if (!setLatestSimWorldTimestamp(tsValue)) {
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Received old timestamp in instruction => Message discarded!");
        XERCES_CPP_NAMESPACE::XMLString::release(&timestamp);
	return;
    }
    ...
    //MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "sentence = '%s'\n", sentence.c_str());
...
//    } else {
//        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "Instruction from a non-owner avatar (%s). So, predavese parser not called for it", internalAvatarId.c_str());
//    }
...
void PAI::processAgentSensorInfo(XERCES_CPP_NAMESPACE::DOMElement * element){
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Received an agent-sensor-info. %d perceptions",perceptionList->getLength() );
	...
	MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Received a perception: %s, %s, %s", sensor, subject, signal );
...
void PAI::processAvatarSignal(XERCES_CPP_NAMESPACE::DOMElement * element) throw (LADSUtil::RuntimeException, LADSUtil::InvalidParamException, std::bad_exception) {
    XMLCh tag[PAIUtils::MAX_TAG_LENGTH+1];
    ...
    unsigned long tsValue = getTimestampFromXsdDateTimeStr(timestamp);
    if (!setLatestSimWorldTimestamp(tsValue)) {
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Received old timestamp in avatar-signal => Message discarded!");
        XERCES_CPP_NAMESPACE::XMLString::release(&timestamp);
	return;
    }
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Got avatar-signal: avatarId = %s (%s), name = %s, timestamp = %s\n", avatarID, internalAvatarId.c_str(), name, timestamp);
...
void PAI::processObjectSignal(XERCES_CPP_NAMESPACE::DOMElement * element) {
    ...
    unsigned long tsValue = getTimestampFromXsdDateTimeStr(timestamp);
    if (!setLatestSimWorldTimestamp(tsValue)) {
        MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Received old timestamp in object-signal => Message discarded!");
        XERCES_CPP_NAMESPACE::XMLString::release(&timestamp);
	return;
    }
    ...
    for(unsigned int i = 0; i < list->getLength(); i++){
        ...
        if (!strcmp(paramName, POSITION_PARAMETER_NAME)) {
            ...
            if (paramValue && strlen(paramValue) > 0) {
                ...
                MAIN_LOGGER.log(LADSUtil::Logger::WARNING, "PAI - The object-signal param '%s' has no value!\n", paramName);
            }
...
void PAI::processMapInfo(XERCES_CPP_NAMESPACE::DOMElement * element, HandleSeq &toUpdateHandles) {
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - processMapInfo(): init."); 
    ...
    if (!strlen(globalPosOffsetStr)) {
         MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - processMapInfo(): got no %s attribute", GLOBAL_POS_Y_ATTRIBUTE); 
    }
    double offset = atof(globalPosOffsetStr);
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI - processMapInfo(): global position offset = %s => offset = %lf", globalPosOffsetStr, offset); 
    ...
    if (!strlen(globalPosXStr)) {
         MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - processMapInfo(): got no %s attribute", GLOBAL_POS_X_ATTRIBUTE); 
    }
    xMin = atof(globalPosXStr);
    xMax = xMin + offset;
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI - processMapInfo(): global position x = %s => xMin = %lf, xMax = %lf", globalPosXStr, xMin, xMax); 
    ...
    yMax = yMin + offset;
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI - processMapInfo(): global position y = %s => yMin = %lf, yMax = %lf", globalPosYStr, yMin, yMax); 
    ...
    for(unsigned int i = 0; i < blipList->getLength(); i++){
        ...
        unsigned long tsValue = getTimestampFromXsdDateTimeStr(timestamp);
        if (!setLatestSimWorldTimestamp(tsValue)) {
            MAIN_LOGGER.log(LADSUtil::Logger::ERROR, "PAI - Received old timestamp in blip => Message discarded!");
            XERCES_CPP_NAMESPACE::XMLString::release(&timestamp);
            continue;
        }
...
//	    MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PAI - processMapInfo(): entityType=%s", entityType); 
...
//	    MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PAI - processMapInfo(): objName=%s", objName); 
...
//            MAIN_LOGGER.log(Util::Logger::INFO, "PAI - processMapInfo(): ownerId=%s (%s)", ownerId, internalOwnerId.c_str()); 
            if (isPetObject) {
...
//		    MAIN_LOGGER.log(Util::Logger::INFO, "PAI - processMapInfo(): before addSpacePredicates"); 
...
//		    MAIN_LOGGER.log(LADSUtil::Logger::INFO, "PAI - processMapInfo(): after addInheritanceLink"); 
        }
...
Handle PAI::addActionToAtomSpace(ActionPlanID planId, const PetAction& action) throw (LADSUtil::RuntimeException, LADSUtil::InvalidParamException, std::bad_exception) {
            ...
            case VECTOR_CODE:
                {
                    ...
                    // TODO: check if string is a valid number? 
                    vectorListLink.push_back(AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(value.x).c_str()));
                    vectorListLink.push_back(AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(value.y).c_str()));
                    vectorListLink.push_back(AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(value.z).c_str()));
            ...
            case ROTATION_CODE:
                {
                    ...
                    HandleSeq rotationListLink; 
                    // TODO: check if string is a valid number? 
                    rotationListLink.push_back(AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(value.pitch).c_str()));
                    rotationListLink.push_back(AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(value.roll).c_str()));
                    rotationListLink.push_back(AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(value.yaw).c_str()));
...
Handle PAI::addActionPredicate(const char* predicateName, const PetAction& action, unsigned long timestamp, ActionID actionId) {
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - execLink = %s \n", TLB::getAtom(execLink)->toString().c_str());
...
void PAI::addPropertyPredicate(std::string predicateName, Handle objectNode, bool propertyValue, bool permanent){
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::FINE, "PAI - Adding predName: %s, value: %d, obj: %s",
...
void PAI::addInheritanceLink(std::string conceptNodeName, Handle subNodeHandle, bool inheritanceValue){
    ...
    if(inheritanceValue){
        MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - Inheritance: %s => %s",
...
bool PAI::addSpacePredicates(bool keepPreviousMap, Handle objectNode, unsigned long timestamp,
    ...
    if (length > 0 && width > 0 && height > 0) {
        ...
        Handle lengthNode = AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(length).c_str());
        Handle widthNode = AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(width).c_str());
        Handle heightNode = AtomSpaceUtil::addNode(atomSpace, NUMBER_NODE, LADSUtil::toString(height).c_str());
        ...
        // For now, assume any object's size does not change in time.
        MAIN_LOGGER.log(LADSUtil::Logger::WARNING, "PAI - addSpacePredicates(): No size information available in mapinfo.");  
        ...
        MAIN_LOGGER.log(LADSUtil::Logger::WARNING, "PAI - addSpacePredicates(): Got last size information available.");  
        }
        if (length <= 0 || width <= 0 || height <= 0) {
            MAIN_LOGGER.log(LADSUtil::Logger::WARNING, "PAI - addSpacePredicates(): No size information available for SL object: %s\n", atomSpace.getName(objectNode).c_str());
    ...
    MAIN_LOGGER.log(LADSUtil::Logger::DEBUG, "PAI - addSpacePredicates - Adding object to spaceServer. name[%s], isAgent[%s], hasPetHeight[%s], isObstacle[%s], height[%f], pet_height[%f], is_pickupable[%s], isSelfObject[%s]", objectName.c_str( ), (isAgent?"t":"f"), (hasPetHeight?"t":"f"), (isObstacle?"t":"f"), height, pet_height, (isPickupable?"t":"f"), (isSelfObject?"t":"f") );
