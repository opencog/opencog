...
void Boltzmannize( Node *n, float *b )
{
    uint i;
...
void CalculateBelief( Node *n, float *x, float *b )
{
    uint i, j;
    float bSum, bDiff, bNorm;
...
float BeliefMSE( uint n, float *b, float *bx )
{
    uint i;
    float diff;
    float mse = 0;
...
void SampleInputFromBelief( Node *n, float *x )
{
    uint i, j;
    float xp[n->ns];    // candidate sample from distribution
    float bx[n->nb];    // belief of current sample
    float bxp[n->nb];   // belief of candidate sample
    float bxmse, bxpmse;
    float accept;       // acceptance ratio

    uint nIts, its;

    // jump distribution variance
    #define JUMPVAR 0.01
    ...
    bxmse = BeliefMSE( n->nb, bx, n->pBelief );

    for( its=0; its < nIts; its++ )
    {
        // generate a new sample x'
        for( i=0; i < n->ns; i++ )
        {
            xp[i] = NormRND(x[i], n->sigma[n->genWinner*n->ns+i]);

            // bound the jumping distribution
            if( xp[i] < 0 ) xp[i] = 0;
            if( xp[i] > 1 ) xp[i] = 1;
        }

        // calculate belief for xp
        CalculateBelief( n, xp, bxp );

        // calculate acceptance ratio
        bxpmse = BeliefMSE(n->nb, bxp, n->pBelief );
        accept = bxmse / bxpmse;
        //printf("bxmse: %f bxpmse: %f. accept: %f\n", bxmse, bxpmse, accept);

        if( accept >= 1 )
        {
            //printf("accepted outright. accept=%0.3f\n", accept);
            for( i=0; i < n->ns; i++ )
            {
                x[i] = xp[i];
            }
            ...
            bxmse = bxpmse;
         } else if( accept >= (float) rand() / RAND_MAX ) {
            //printf("accepted with p=%0.3f\n", accept);
            for( i=0; i < n->ns; i++ )
            {
                x[i] = xp[i];
            }
