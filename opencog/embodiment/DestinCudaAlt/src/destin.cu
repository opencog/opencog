...
Destin * InitDestin( int ni, int nl, int *nb, int nMovements )
{
    ...
    // get integer sq root of ni for lowest layer.  asssumes 2^n
...
/*
            ...
            for( j=0; j < d->nBeliefs; j++ )
            {
                if( isnan(d->belief[j]) )
                {
                    printf("nan in the dataset shoo fly shoo.  it = %d\n", i);
                    PrintNetwork( d, d->dataSet + i*inputFrameSize );
                    exit(1);
                }
            }
*/
