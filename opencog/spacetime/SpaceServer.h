...
public:
     ...
     */
    Handle addOrGetSpaceMap(const std::string timedomain, octime_t timestamp, std::string _mapName, int _xMin, int _yMin, int _zMin, int _xDim, int _yDim, int _zDim, int _floorHeight);
    ...
    void removeSpaceInfo(Handle objectNode, Handle spaceMapHandle, const std::string timedomain, octime_t timestamp = 0);
     ...
     */
    void addBlocksListPredicateToEntity(opencog::spatial::BlockEntity* _entity, const std::string timedomain, const octime_t timeStamp, Handle spaceMapHandle);

    /**
     * add properties predicate link to an entity node when there is a change
     * this including addBlocksListPredicateToEntity
     */
    void updateBlockEntityProperties(opencog::spatial::BlockEntity* entity,const std::string timedomain, octime_t timestamp,Handle spaceMapHandle);

    void updateBlockEntitiesProperties(const std::string timedomain, octime_t timestamp, HandleSeq &toUpdateHandles, Handle spaceMapHandle);
