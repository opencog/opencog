...
namespace moses {
  ...
  // out            - deme (where to store the instances) 
  template<typename Out>  
  void vary_n_knobs(const eda::field_set& fs,eda::instance& inst, int n, int starting_index, Out& out) {
    if(n<0)
      return;
  ...
  // n              - distance
  // starting_index - position of a field to be varied
  inline long long count_n_changed_knobs_from_index(const eda::field_set& fs,int n, int starting_index) {
    if(n<0)
      return 0;
