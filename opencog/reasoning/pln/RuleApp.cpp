...
namespace reasoning
{
const vtree& RuleApp::getVtree() const { vt_result = compute(NULL).value;
	return vt_result; }
...
BoundVertex RuleApp::compute(const vector<Vertex>& h, Handle CX) const
{
	...
	RuleApp::RuleApp(//iAtomTableWrapper *_destTable,
			...
			:	Rule(_root_rule->destTable, false, true, "Inference Pathway"),
				root_rule(_root_rule), result((Handle)NULL), arg_changes_since_last_compute(true)
	{ 
	...
	bool RuleApp::Bind(int arg_i, VtreeProvider* arg) const
	{
		if (args[arg_i])
		{
			...
			if (!vtw || vtw->val != vtree((Handle)NULL))
			{
				assert(0);
				return false;
			}
...
//		assert(dynamic_cast<VtreeProviderWrapper*>(ra) != /*(VtreeProviderWrapper*)*/NULL)
	...
	set<Rule::MPs> RuleApp::o2iMetaExtra(meta outh, bool& overrideInputFilter) const
	{
		...
		{
			const RuleApp* ra;
			if (ra = dynamic_cast<const RuleApp*>(*ai)) //If the arg is a RuleApp, take its input vector
			{
