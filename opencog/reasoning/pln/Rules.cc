...
printf("CreateVar Added node as NEW: %s / [%d]\n", varname.c_str(), ret);
...
Rule::Rule(reasoning::iAtomTableWrapper *_destTable,
			...
			bool _computable,
			std::string _name)
: destTable(_destTable), name(_name), freeInputArity(_FreeInputArity),
computable(_computable), priority(10.0f), RULE_INPUT_ARITY_MAX(15)
{
}
...
printf("Crispu.o2i: OK! Solution vector size=%d\n", ret.size());
...
/// should have different binds for left and right!!!
		...
		if (unifiesTo(thm_target, *outh, binds, binds, true))
		{
			...
			for (int i = 0; i < new_thm_args.size(); i++)
...
printf("ANDRule:: getLargestIntersection");
		...
		{
printf("Y ANDRule::compute Di size = %d\n", Di.size());			
...
bool ExpandEvaluationLinks(vtree& target, iAtomTableWrapper* destTable)
{
	...
	for(vtree::iterator i = target.begin(); i != target.end(); i++)
		if (nm->inheritsType(nm->getType(v2h(*i)), CONCEPT_NODE))
		{
			...
			sprintf(begin_stamp_s, "%ul", max(0,now_stamp-now_interval));
