...
#include "../reasoning/util/Ptlatom.h"
...
Handle exec(vector<Handle>& hs)
{
  try
  {
	...
	nocase_string procedure_name = nm->getName(hs[0]);
	
//	puts(("Launching " + procedure_name).c_str());

	// hs[1] should be list link
	vector<Handle> args = nm->getOutgoing(hs[1]);
	const int N = nm->getArity(hs[1]);
//	printf("%d arguments\n", N);
...
//		TruthValue::TRIVIAL_TV()));
	...
	return nm->addNode(NUMBER_NODE, strdup(buff),
...
/*	HandleTemporalPairEntry* interval = TimeServer::getInstance()->getTimeLag(t);
	...
	*/
	...
	if (inheritsType(nm->getType(t), NODE))
		return (timeUnit)atof(nm->getName(t).c_str());
...
Handle TimeApparatus::intervalStart(Handle interval)
{
	...
	vector<Handle> intervalOutgoingSet = nm->getOutgoing(interval);
    int arity = nm->getArity(interval); 
...
//    Atom* atom = TLB::getAtom(h);
	return (nm->getType(h) == NUMBER_NODE);
}
...
//       Node* numberNode = (Node*) TLB::getAtom(h);
	return atoi(inheritsType(nm->getType(h), NODE) ? nm->getName(h).c_str() : 0);
}
...
//		return false;
    ...
    Handle arglist1 = nm->addLink(LIST_LINK, args_start, TruthValue::NULL_TV(), false, true);
    Handle arglist2 = nm->addLink(LIST_LINK, args_end, TruthValue::NULL_TV(), false, true);
							...
							bool tv)
{
	...
	argsEval.push_back(nm->addNode(PREDICATE_NODE, predName, TruthValue::NULL_TV(), false,true));
	argsEval.push_back(nm->addLink(LIST_LINK, argsList, TruthValue::NULL_TV(), false,true));
	
	return nm->addLink(EVALUATION_LINK, argsEval,
			SimpleTruthValue((float)tv, MAX_TV_COUNT),
			false,true
			);
}
...
struct moment_equal : public binary_function<reasoning::BoundVertex, timeUnit, bool>
{
	...
	{
		HandleTemporalPairEntry* interval = TimeServer::getInstance()->getTimeLag(boost::get<Handle>(h.value));
		return (interval
				? (interval->handleTemporalPair.getTemporal()->getA() == t)
				: false);
	}
};
