...
//currentDebugLevel = 5; 
   ...
   // L(ClassServer::getTypeName(EQUIVALENCE_LINK), EQUIVALENCE_LINK);
	...
	L("__OR", __OR); // defined in Ptlatom.h
	L("__AND", __AND); // defined in Ptlatom.h
	L("__NOT", __NOT); // defined in Ptlatom.h
	L("R", RESTRICTOR); // defined in Ptlatom.h
	L("InstanceOf", INSTANCEOF_R); // defined in Ptlatom.h
	L("In", IN_R); // defined in Ptlatom.h
	L("Contains", HAS_R); // defined in Ptlatom.h
