...
BITNode::BITNode(		BITNodeRoot* _root,
	...
	raw_target(_target)
	{
	  ...
	  } catch(string s) { printf("EXCEPTION IN BITNode::BITNode! $s\n",s.c_str());
		  getc(stdin); getc(stdin); throw; }
	  catch(...)  { printf("EXCEPTION IN BITNode::BITNode!\n");
		  getc(stdin); getc(stdin); throw; }
...
BITNode* BITNode::CreateChild(int target_i, Rule* new_rule,
...
#if 1
void BITNode::EvaluateWith(int arg_i, VtreeProvider* new_result)
{
	AtomSpace *nm = CogServer::getAtomSpace();
//	tlog(-1, "ARG %d:\n", arg_i);
...
int BITNode::tlog(int debugLevel, const char *format, ...) const
	{
	    if (debugLevel > 0) return 0;
		 ...
		 printf("%d %d/%d (%d) [(%ld)] (%s): ", (bigcounter?(++test::bigcount):depth), root->exec_pool.size(), root->InferenceNodes, haxxUsedProofResources,
//			(test::LOG_WITH_NODE_ID ? ((int)this) : 0),
			(long)this,
			(rule ? (rule->name.c_str()) : "(root)"));
		
		if (test::logfile)
			fprintf(test::logfile, "%d %d [(%ld)] (%s): ", depth, root->InferenceNodes,
//			(test::LOG_WITH_NODE_ID ? ((int)this) : 0),
			(long)this,
			(rule ? (rule->name.c_str()) : "(root)"));
