...
# define LOG(l, m) {                 \
    if(l <= Log::masterLogLevel) {      \
        Log* _lg = Log::getLog(LOG_STR);         \
        if(! _lg) { _lg=new Log(LOG_STR); Log::addChannel(_lg); }  \
        if(_lg->getLevel() >= l) {          \
          std::string _x(m); \
          _lg->log(l, _x.c_str());           \
        } \
...
enum LogLevelSym {
    ...
    /// Critical, possibly fatal errors
    LOG_ERROR = 0,

    /// Important warnings
    LOG_WARN,

    /// Important information about program state or progress.
    LOG_NOTIFY,

    /// Details about program state or progress, which may be useful for users
    LOG_DETAIL,

    /** Verbose program debugging, useful for developers, and details about VOS
        state or progress.
    */
    LOG_DEBUG,
...
public:
    ...
    */
    ...
    /** Write a line to the log stream.  You probably want to be
        ...
        @internal
    */
    void log(int level, const std::string& message,bool prefix=true);
    ...
    /** Log to a particular channel, which will be created if it does
        ...
        @internal
     */
    static void log(const char* channel, int level, const std::string& message,bool prefix=true);
    static void log(const std::string& channel, int level, const std::string& message,bool prefix=true);
...
#define TheLog (Log::Instance())

#endif // VOS logger disable
