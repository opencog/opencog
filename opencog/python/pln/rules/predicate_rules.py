...
# That chapter handles some cases of PLN higher order logic that don't require quantifiers
...
# Cases with variables can be handled by EvaluationToMemberRule (in other file). But it would be better to do it implicitly, so you don't have the separate steps of converting from FOL to HOL and back again.
...
'''
class EvaluationImplicationRule(rules.Rule):
...
'''
Similarity substitution Rule is equivalent to combining that rule with Abduction
...
Maybe you should just use Mem2Inh and Inh2Mem instead (or even convert everything into inhlinks to see what would happen!)

Eval2Implication should be a special case but would only work for one-argument versions of the predicates...
