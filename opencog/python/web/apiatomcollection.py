...
class AtomCollectionAPI(Resource):
    ...
    def get(self):
        ...
        :return result: Returns a JSON representation of an atom list. Example:
        {  'result':
        {  'complete': 'true', 'skipped': 'false', 'total': 10,
           'atoms':
    ...
    def post(self):
        ...
        :param type: (required) Atom type, see http://wiki.opencog.org/w/OpenCog_Atom_types
        :param name: (required for Node types, not allowed for Link types) Atom name
        :param truthvalue: (required) TruthValue, formatted as follows:
            ...
            details TruthValue parameters, formatted as follows:
                strength
                count
        :param outgoing: (optional) The set of arguments of the relation, formatted as a list of Atom handles
            (only valid for Links, not nodes), see http://wiki.opencog.org/w/Link#Incoming_and_Outgoing_Sets

        Example:

        Node: { 'type': 'ConceptNode', 'name': 'Frog',
                'truthvalue': { 'type': 'simple', 'details': { 'strength': 0.8, 'count': 0.2 }}}

        Link: { 'type': 'InheritanceLink', 'outgoing': [1, 2],
