...
def calculateCenterMass(dist):
	...
	if dist[init_segment] == dist[end_segment] and dist[init_segment]!=0:
	  ...
	  centroid_t = centroid_t + area_rectangle*((end_segment+init_segment)/float(2))
	  centroid_c = centroid_c + area_rectangle*(dist[init_segment]/float(2))
	...
	#we calculate its area, and its centroid ponderated by the area
	  ...
	  total_area = total_area + area_triangle
	  centroid_t = centroid_t + area_triangle*((((end_segment-init_segment)/float(3))*2)+init_segment)
	  centroid_c = centroid_c + area_triangle*(dist[end_segment]/float(3))
	  ...
	  total_area = total_area + area_triangle
	  centroid_t = centroid_t + area_triangle*(((end_segment-init_segment)/float(3))+init_segment)
	  centroid_c = centroid_c + area_triangle*(dist[init_segment]/float(3))
	...
	#it is both a rectangle and a triangle
	  ...
	  total_area = total_area + area_rectangle
	  centroid_t = centroid_t + area_rectangle*((end_segment+init_segment)/float(2))
	  centroid_c = centroid_c + area_rectangle*(rectangle_height/float(2))
	  ...
	  if dist[end_segment]>dist[init_segment]:
	    ...
	    total_area = total_area + area_triangle
	    centroid_t = centroid_t + area_triangle*((((end_segment-init_segment)/float(3))*2)+init_segment)
	    centroid_c = centroid_c + area_triangle*(((dist[end_segment]-dist[init_segment])/float(3))+dist[init_segment])
	  else:  
	    area_triangle = ((triangle_height)*(end_segment-init_segment))/float(2)
	    total_area = total_area + area_triangle
	    centroid_t = centroid_t + area_triangle*(((end_segment-init_segment)/float(3))+init_segment)
	    centroid_c = centroid_c + area_triangle*(((dist[init_segment]-dist[end_segment])/float(3))+dist[end_segment])
	#we move forward
