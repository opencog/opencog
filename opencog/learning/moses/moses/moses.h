...
namespace moses {
  ...
  struct metapopulation : public set<behavioral_scored_tree,
    ...
    metapopulation(opencog::RandGen& _rng, const combo_tree& base,const combo::type_tree& t,
      ...
      rng(_rng),type(t),simplify(&si),score(sc),bscore(bsc),optimize(opt),params(pa),
      _n_evals(0),_best_score(worst_possible_score),_rep(NULL),_deme(NULL)
    ...
    const std::vector<combo_tree>& best_trees() const { return _best_trees; }
    ...
    bool expand(int max_evals,
        ...
        representation rep(*simplify,exemplar->first,type,rng, os,perceptions,actions);
    ...
    bool create_deme(const operator_set* os=NULL,
        ...
        //do representation-building and create a deme (initially empty)
        _rep = new representation(*simplify,_exemplar->first,type,rng, os,perceptions,actions);
    ...
    int optimize_deme(int max_evals,int max_for_slice,const tree_score& max_score) {
        ...
        int n = optimize(*_deme,count_based_scorer<Scoring>(score,*_rep,exemplar_complexity, rng), max_evals-n_evals());
  ...
  void moses(metapopulation<Scoring,Domination,Optimization>& mp,
	     int max_evals,score_t max_score,
             ...
             const combo_tree_ns_set* actions,
             ordered_programs& op) {
    moses(mp,max_evals,tree_score(max_score,worst_possible_score.second),os,perceptions,actions,op);
  }
  ...
  void moses_sliced(metapopulation<Scoring,Domination,Optimization>& mp,
	     int max_evals,score_t max_score,
             ...
             const combo_tree_ns_set* actions,
             ordered_programs& op) {
    moses_sliced(mp,max_evals,tree_score(max_score,worst_possible_score.second),os,perceptions,actions,op);
  }
