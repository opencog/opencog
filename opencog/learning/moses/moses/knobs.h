...
namespace moses {
  ...
  struct logical_subtree_knob : public knob_with_arity<3> { 
    ...
    logical_subtree_knob(combo_tree& tr,combo_tree::iterator tgt,combo_tree::iterator subtree)
      ...
      for (combo_tree::sibling_iterator sib=tgt.begin();sib!=tgt.end();++sib) {
	if (_tr->equal_subtree(combo_tree::iterator(sib),subtree) || 
	    _tr->equal_subtree(combo_tree::iterator(sib),negated_subtree.begin())) {
	  _loc=sib;
	  _current=present;
	  _default=present;
	  return;
  ...
  struct ant_action_subtree_knob : public knob_with_arity<4> { 
    ...
    ant_action_subtree_knob(combo_tree& tr,combo_tree::iterator tgt,combo_tree::iterator subtree)
