...
// ///////////////////////// Scoring for Passenger-style data /////////////////////////
  ...
  bool bool_evaluate(const State& bindings,iter it) { 
    ...
    if (!(*it==id::boolean_if || *it==id::logical_and || *it==id::logical_or || 
          *it==id::logical_not || *it==id::logical_true || *it==id::logical_false))  {
...
//    complexity_t cmin=get_complexity(t.begin());
    ...
    void compute_behavior(const combo_tree& t,behavioral_score& bs) const {
      int i=0;
      for (CaseSeq::const_iterator c=_cases.begin();c!=_cases.end();++c,++i) 
	bs[i]=(bool_evaluate(*c,t.begin())!=(*c)[arity()]); 
    }


    int arity() const { return _cases.front().size()-1; }
    int number_of_cases() const { return _cases.size(); }
