...
;
; -----------------------------------------------------------------------
; cog-for-each atom-type proc atom-list
;
...
;
; Exmaple usage:
; (cog-for-each 'ConceptNode display (list (cog-new-node 'ConceptNode "hello")))
; 
; See also: cgw-filter-atom-type, which does the same thing, but for wires.
;
(define (cog-for-each atom-type proc atom-list) 
	(define rc #f)
	(cond 
		((null? atom-list) #f)
		((eq? (cog-type (car atom-list)) atom-type) 
			...
			(if (eq? #f rc) 
				(cog-for-each atom-type proc (cdr atom-list))
				rc
			)
		) 
		(else (cog-for-each atom-type proc (cdr atom-list))
		)
	)
)
...
(define (cog-map-chase-link link-type endpoint-type dbg-lmsg dbg-emsg proc anchor)
	(define (get-endpoint w)
		...
		; cog-for-each returns the return value from proc, we pass it on
		; in turn, so make sure this is last statement
		(cog-for-each endpoint-type proc (cog-outgoing-set w))
	)
	(if (not (eq? '() dbg-lmsg)) (display dbg-lmsg))
	; cog-for-each returns the return value from proc, we pass it on
	; in turn, so make sure this is last statement
	(cog-for-each link-type get-endpoint (cog-incoming-set anchor))
)
