/*
 * src/AtomSpace/TemporalTable.cc
 *
...
void TemporalTable::load(FILE *fp, HandleMap<Atom *> *conv) throw (InconsistenceException)
{
    ...
    for (int i = 0; i < size; i++){
        ...
        for (int j = 0; j < setSize; j++) {
            ...
            add(TLB::getHandle((const Atom *) conv->get(oldHandle)),t);
        }
    }
}

void TemporalTable::save(FILE *fp) {
    // writes table size (number of temporal entries)
    int size = temporalMap->getCount();
    ...
    while (currentEntry != NULL){
        ...
        HandleSet* hs = temporalMap->get(t);
