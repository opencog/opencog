...
private:
    ...
    /**
     * indicates whether DynamicStatisticsAgent should be used for atoms inserted in 
     * this table or not.
     */
    bool useDSA;
    ...
    void removeFromIndex(Atom *, std::vector<Handle>&, int, int) throw (RuntimeException);
    ...
    /**
     * Extracts atoms from the table. Table will not contain the extracted atoms anymore, but 
     * they will not be deleted. Instead, they are returned by this method.
     * 
     * Note: The caller is responsible for releasing the memory of both the returned list and the 
     * refered Atoms inside it. 
     *
     * @param The atom to be extracted.
     * @param Recursive-removal flag; if set, the links in the incoming set will also be extracted.
     * @return A list with the Handles of all extracted Atoms. 
     */
    HandleEntry* extract(Handle, bool recursive = false);

    /**
     * Removes the previously extracted Handles (using the extract method) from this table. 
     * @param The list of the Handles previously extracted.
     * 
     * NOTE: This method also frees the memory of both list and Atom objects corresponding to the Handles inside the list. 
     */
...
public:
     ...
     * @return The newly created iterator.
     */
    HandleIterator* getHandleIterator(Type, bool subclass = false, VersionHandle vh = NULL_VERSION_HANDLE);
     ...
     */
    HandleEntry* buildSet(Type, bool, Handle(AtomTable::*)(Type) const, int) const;
     ...
     * NOTE: Does not apply the new predicate index to the atoms inserted previously in the AtomTable.
     */
    void addPredicateIndex(Handle, PredicateEvaluator*) throw (InvalidParamException);
    ...
    /**
     * Returns the Predicate evaluator for a given GroundedPredicateNode Handle, 
     * if it is being used as a lookup index. Otherwise, returns NULL.
     */
    PredicateEvaluator* getPredicateEvaluator(Handle gpnHandle) const;
    
    /**
     * Returns a list of handles that matches the GroundedPredicateNode with 
     * the given name (id).
     * @param the id of the predicate node.
     * @param VersionHandle for filtering the resulting atoms by context. NULL_VERSION_HANDLE indicates no filtering
     **/
    HandleEntry* findHandlesByGPN(const char*, VersionHandle = NULL_VERSION_HANDLE) const;
     ...
     * @param VersionHandle for filtering the resulting atoms by context. NULL_VERSION_HANDLE indicates no filtering
     **/
    HandleEntry* findHandlesByGPN(Handle, VersionHandle = NULL_VERSION_HANDLE) const;
     ...
     * optionally).
     */
    HandleEntry* getHandleSet(Type, Type, bool subclass = false, bool targetSubclass = false) const;
     ...
     * their outgoing set.
     */
    HandleEntry* getHandleSet(Handle, Type type = ATOM, bool subclass = true) const;
     ...
     */
    HandleEntry* getHandleSet(const std::vector<Handle>&, Type*, bool*, Arity, Type type = ATOM, bool subclass = true) const;
     ...
     */
    HandleEntry* getHandleSet(const char*, Type type = ATOM, bool subclass = true) const;
     ...
     */
    HandleEntry* getHandleSet(const char*, Type, Type type = ATOM, bool subclass = true) const;
     ...
     * criteria in their outgoing set.
     */
    HandleEntry* getHandleSet(const char**, Type*, bool*, Arity, Type type = ATOM, bool subclass = true) const throw (RuntimeException);
     ...
     * criteria in their outgoing set.
     */
    HandleEntry* getHandleSet(Type*, bool*, Arity, Type type = ATOM, bool subclass = true) const;
     ...
     */
    HandleEntry* getHandleSet(AttentionValue::sti_t, AttentionValue::sti_t upperBound = 32767) const;
     ...
     * @param Recursive-removal flag; if set, the links in the incoming set will also be removed.
    ...
    /**
     * Decays importance of all atoms in the table, reindexing importanceIndex
     * accordingly.
     */
