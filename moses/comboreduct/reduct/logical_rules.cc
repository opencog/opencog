...
void subtree_to_enf::reduce_to_enf::operator()(sib_it it)
{
    ...
    {
        if (!moses3::is_sorted(make_counting_iterator(p.begin()),
                                      ...
                                      const subtree_set& command)
{
    ...
    OC_ASSERT(moses3::is_sorted(dominant.begin(), dominant.end(), comp),
    ...
    OC_ASSERT(moses3::is_sorted(dominant.begin(), dominant.end(), comp),
              "dominant subtree_set should be sorted (reduce)");
    OC_ASSERT(moses3::is_sorted(command.begin(), command.end(), comp),
...
subtree_to_enf::reduce_to_enf::reduce_and(sib_it current,
                                          ...
                                          const subtree_set& command)
{
    moses3::erase_set_intersection(tree_eraser(tr),
                                    make_counting_iterator(current.begin()),
                                    make_counting_iterator(current.end()),
    ...
    if (!moses3::has_empty_intersection
        (make_counting_iterator(current.begin()),
         make_counting_iterator(current.end()),
    ...
    do {
        ...
        if (!moses3::is_sorted(make_counting_iterator(current.begin()),
        ...
        for (sib_it child = current.begin(); child != current.end(); ) {
            ...
            OC_ASSERT(moses3::is_sorted(handle_set.begin(),handle_set.end(),comp),
            ...
            case Keep:
                ...
                for (sib_it sib = child.begin(); sib != child.last_child(); ++sib)
                    moses3::erase_set_difference(subtree_set_eraser(res),
                                                  res.begin(), res.end(),
