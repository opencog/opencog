...
class ConnectConflictRandom(BaseConnector):
    ...
    def __connect_links_simple(self, decided_atoms, new_blended_atoms):
        ...
        """
        # In ConnectConflictRandom, number of result atoms is only one.
        if len(new_blended_atoms) < 1:
            ...
            raise UserWarning(
                ...
                'ConnectConflictRandom can handle one atom in each execute.'
            )
    ...
    def link_connect_impl(self, decided_atoms, new_blended_atoms, config_base):
        ...
        check_strength_diff_threshold = BlendConfig().get_str(
            self.a, "connect-strength-diff-limit", config_base
        )
        check_confidence_above_threshold = BlendConfig().get_str(
        ...
        try:
            self.strength_diff_limit = \
                float(check_strength_diff_threshold)
            self.confidence_above_limit = \
                float(check_confidence_above_threshold)
        except (TypeError, ValueError):
            raise UserWarning(
                "Can't parse threshold value:: "
                "{strength: {0}, confidence: {1}}".format(
                    str(check_strength_diff_threshold),
                    str(check_confidence_above_threshold))
            )
